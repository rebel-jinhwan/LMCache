...
def test_discovers_the_registered_hnd_format() -> None:
    """The 5-D views resolve to NL_X_TWO_NB_NH_BS_HS -- no new format needed."""
    connector = _connector_with_attributes()
    assert int(connector.engine_kv_format) == int(EngineKVFormat.NL_X_TWO_NB_NH_BS_HS)


def test_geometry_matches_the_native_tensors() -> None:
    """Discovery reads the geometry through the squeezed axis."""
    connector = _connector_with_attributes()
