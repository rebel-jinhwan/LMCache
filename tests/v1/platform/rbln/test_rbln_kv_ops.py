...
Pure torch, so no RBLN hardware is needed.
"""
...
def _paged_layers(fill_random: bool = True) -> list[torch.Tensor]:
    """Per-layer HND KV, the 5-D layout the detector normalizes RBLN to."""
...
def _transfer(
    ...
    engine_kv_format: EngineKVFormat = EngineKVFormat.NL_X_TWO_NB_NH_BS_HS,
) -> None:
...
def test_chunk_is_head_major_not_token_major() -> None:
    ...
    assert all(
        torch.equal(
            ...
            layers[li][kv, ti // BLOCK_SIZE, hi, ti % BLOCK_SIZE],
        )
        for kv in (0, 1)
    ...
    assert not all(
        torch.equal(
            chunks[0][kv, li, ti],
            layers[li][kv, ti // BLOCK_SIZE, :, ti % BLOCK_SIZE, :].reshape(
                NUM_HEADS * HEAD_SIZE
            ),
        )
        for kv in (0, 1)
