...
// One async device<->host copy. The device address is an RBLN tensor's
// ``data_ptr()``; the host address is an LMCache chunk's. The rebel handle is
// drained by the caller's single device sync, so it is not kept.
...
void head_major_block_kv_transfer(const std::vector<at::Tensor>& paged_layers,
  ...
  // Geometry comes from the first layer; every other layer is required to
  // match, so one set of strides addresses all of them.
  ...
  // Checked up front so a geometry mismatch is a clean error rather than a
  // silent out-of-bounds DMA: nothing below re-derives shapes per operand.
  ...
  // When a chunk holds exactly one block, a whole (K|V, layer) block is
  // contiguous on both sides, so it moves in one DMA instead of one per head.
  ...
  // Drain every issued transfer before returning: the rebel runtime does no
  // auto-sync on host read, and the caller must see a fully materialized chunk
  // (D2H) or a fully written KV cache (H2D). One sync covers the whole burst.
  if (device_id >= 0) {
