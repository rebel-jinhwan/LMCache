...
//
// ``direction`` crosses as a plain int rather than a bound enum:
// ``TransferDirection`` is already registered by ``lmcache.c_ops``, and
// pybind11 rejects a second registration of the same C++ type in one
// interpreter.
