...
#: The only layout the head-major path is validated for: per-layer HND with
#: the K/V pair leading, which is what the detector normalizes RBLN KV to.
_SUPPORTED_FORMAT = EngineKVFormat.NL_X_TWO_NB_NH_BS_HS
...
class RblnDeviceOps(DeviceOps):
    ...
    def multi_layer_block_kv_transfer(
        ...
        Args:
            paged_buffer_ptrs_tensor: Per-layer HND KV tensors,
                ``[2, NB, NH, BS, HS]``.
